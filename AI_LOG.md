...
**Addendum de la ronda 2 (auditoría de `15a5fa8`, APROBADO CON OBSERVACIONES):** la regla de arriba se puso a prueba sola y mostró su filo. R2 corrigió las tres cifras, partió la tabla y pegó las salidas completas —todo verificado re-corriendo los instrumentos—, pero apareció un error nuevo (H-6) **exactamente en el único lugar donde la regla no se puede aplicar**: `detectar_huecos.py` imprime los huecos intermedios truncados en seis, con un `... (+N mas)`. De las dos frases del parte que citan esos rangos elididos, una salió exacta (`BCB/deuda_externa`, 2022-S2..2025-S2) y la otra no (`ASFI/poa_seguimiento`: el parte puso 2015-Q2..**2026-Q3** cuando el último hueco es **2024-Q3**; 2026-Q3 es el último período *observado*, el que hace que la serie esté al día). El número se tomó de la línea de al lado, que era lo único visible.

La lección práctica que agrega esta ronda: **cuando un instrumento elide su propia salida, esa elisión es el punto ciego de la auditoría, y hay que ir a buscar la salida que no elide antes de escribir sobre ella.** Acá existía y estaba a un flag de distancia: `detectar_huecos.py --format json` emite `intermediate_gaps` íntegro. Ese volcado fue el sondeo que encontró H-6. Corolario para el diseño de instrumentos: un reporte pensado para ser citado debería ofrecer siempre una salida sin truncar, porque el resumen de consola invita a rellenar lo que oculta.
